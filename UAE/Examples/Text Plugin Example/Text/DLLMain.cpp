...
// of the newly imported data, the length of the data, and the index of the extension.  Your plugin should convert the data to the original format, and return a dynamically
...
__declspec(dllexport) byte* __cdecl ImportData(byte *data,DWORD *len,DWORD extension)
{
...
// any data that you intend to use later. The extension DWORD will tell you which format the user selected.  You should set the len variable to the size in bytes of the data
// array.
__declspec(dllexport) byte* __cdecl ExportData(DWORD *len,DWORD extension)
{
